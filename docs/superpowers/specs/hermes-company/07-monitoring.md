...
>
> **用途**：定义三层监控防御机制——被动日志、异常通知（飞书）、硬熔断（自动停），防止任何 agent 陷入死循环而老板无感知。
>
> **当前状态**：三层监控均为规划设计，尚未实现。实际日志为 `company/logs/dispatcher-<项目>.log` 纯文本格式，未实现结构化 JSONL、阈值检测、熔断逻辑。
...
- 日志位置（规划）：`workspace/company/logs/agent-YYYY-MM-DD.jsonl`
- 日志位置（当前实际）：`company/logs/dispatcher-<项目>.log`（纯文本，由 dispatcher-run.sh 写入）
...
- 命中阈值 → 写入 `company/pm-state/alerts.jsonl`，PM 轮询读到后通过 `send_message` 推飞书
