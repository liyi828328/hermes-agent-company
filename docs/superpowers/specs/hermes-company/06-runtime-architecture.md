...
- 启动方式：`hermes --profile pm-agent gateway run --replace`（hermes 内置 gateway 前台常驻模式）
- 可选升级：`hermes --profile pm-agent gateway install` 注册为 launchd/systemd 系统服务（开机启动、挂了自动重启），当前未使用
- 唯一对话方：老板（飞书用户授权）
- 职责：接需求、写 PRD、汇报、简报、回答查询、读 alert 推飞书
- 上下文轻量：不做调度/监控，session 短，响应快
- 飞书凭证：`FEISHU_APP_ID` + `FEISHU_APP_SECRET` 配置在 profile 的 `.env` 中（规划中，当前未实现）
- 飞书订阅方式：长连接（WebSocket），无需公网回调地址（规划中，当前未实现）

## Dispatcher Agent — 常驻

- 实现：独立的 Hermes profile（`hermes profile create dispatcher-agent`）
- 启动方式：**不常驻进程**，通过 Hermes 内置 cron 每 5 分钟触发扫描脚本
- cron job 作为独立定时任务运行，由 `company/scripts/dispatcher-poll.sh` 触发
- 扫描脚本：`company/scripts/dispatcher-poll.sh`
- 锁文件：`company/pm-state/dispatcher-<项目代号>.lock`（每项目独立锁，防止重复触发）
- 不对人说话，纯自动化引擎
- 工作流程：
  1. cron 每 5 分钟触发扫描脚本
  2. 脚本检查锁文件——有锁则跳过
  ...
  7. 没变化 → 静默退出
- 锁文件保护：检查锁文件中记录的 PID 是否存活，进程已死则清理残留锁
...
PM ↔ Dispatcher：company/pm-state/priorities.md（项目优先级）
```

### Dispatcher 流水线脚本

Dispatcher 通过 `company/scripts/dispatcher-run.sh` 阶段化执行项目流水线，每个项目一次完整流水线：

1. 架构设计（spawn Architect）
2. 等待架构审批（轮询 architecture.md status）
3. 拆任务 + 开发（spawn Coder）
4. 代码审查（spawn Reviewer，reject 则闭环修复）
5. Merge + 集成测试
6. QA 验收（spawn QA，失败则闭环修复）
7. 文档编写（spawn Doc）

流水线支持断点续跑：每阶段完成后写信号文件，重启后跳过已完成阶段。

`company/scripts/dispatcher-poll.sh` 负责定时扫描所有项目，对未完成的项目后台启动 `dispatcher-run.sh`。
